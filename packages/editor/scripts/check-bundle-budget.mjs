...
const distUrl = new URL(`../${outputDirectory}/`, import.meta.url);
...
if (!entryMatch || initialJs.length === 0) {
  throw new Error(
    `Could not identify the production entry chunks in ${outputDirectory}/index.html.`,
  );
}
...
console.log(
  `${integrityOnly ? "Bundle integrity" : "Bundle budgets"}: entry ${(entryBytes / 1024).toFixed(1)} KiB; initial JS ${(initialBytes / 1024).toFixed(1)} KiB across ${initialJs.length} chunks; WASM ${(wasmBytes / 1024 / 1024).toFixed(2)} MiB.`,
);
